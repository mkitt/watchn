...
/* Internal */
/* So many loops, begging for some optimization. A Map might be a good replacement */
  ...
  if (!isWatched) {
    fs.watchFile(item, {persistent: true, interval: 50}, function (curr, prev) {
      fs.stat(item, function (err, stats) {
        if (err) {
          self.softremove(item); // THIS IS CLOSER, might just need to strip from arrays?
          // throw new Error(err);
        } else {
...
// just a test to see if it's gone.
...
Watchn.prototype.modifiedDirectory = function(dir, rule) {
  // this will work for adding directories, needs to also work for removing
