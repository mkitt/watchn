...
Watchn.prototype.execute = function(task, options, reporter_name, growlPass, growlFail) {
  if (!task || !options) {
  ...
  if (options.curr > options.prev) {
    exec(task, function(error, stdout, stderr) {
      ...
      self.notify(report.msg, {name: report.name || task, msg: report.gmsg})
    })
  }
}
...
- verbosity (look at npm - "silent", "info", "verbose", "silly")
  ...
  - silly: everything
 */
Watchn.prototype.notify = function(msg, growl) {
  if (!this.silent) {
    console.log(msg)
