...
var Watchn = module.exports = function Watchn(silent) {
  ...
  // this.reporters.add('basic', new Reporter())
}

// growl.showPass, growl.showFail, growl.msgPass, growl.msgFail
Watchn.prototype.execute = function(task, options, reporter_name, log, growl) {
  if (!task || !options) {
    throw new Error('watchn.execute requires a task and options to be passed in')
  }
  var key = reporter_name || 'basic'
  // maybe only pass this in the else statement
  ...
  if (options.curr > options.prev) {
    exec(task, function(error, stdout, stderr) {
      if (error !== null) {
        ...
        // Probably doesn't need to go through a reporter
      } else {
        // notify('markup generated');
        // something like notify(reporter.report(...)
      }
    })
  }
...
/*
  ...
  - log === console default is true (silent will override)
  ...
  - after parsing, it should return the results back here for notifications
  ...
  - Flip flop console and growl in arguments
  - Show time stamp (probably should come from the executable)
  - verbosity (look at npm - "silent", "win", "error", "warn", "info", "verbose", "silly")
  - Test this stuff!

  watchn.xwatch('test', [tests, libs], function(options) {
    watchn.execute('make test', options, 'expresso', true, growl)
  })
...
Watchn.prototype.getReporter = function(key) {
  return (this.reporter.hasKey(key)) ? this.reporter.get(key) : this.createReporter(key);
}

Watchn.prototype.createReporter = function(key) {
  // look in the paths
  // save to reporters map
  // return it
  return null;
}
...
// Output information about what watchn is doing in the background
