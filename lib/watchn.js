...
(function () {
  ...
  // make a better deep copy..
  function copyWatched() {
    var copy = [];
    
    for (var i = 0, len = watched.length; i < len; i += 1) {
      var o = watched[i];
      copy.push({file: o.file, action: o.action});
    }
    return copy;
  }
...
// ----------------------------------------------------------------------------  
  ...
  // refactor this.
  function reload() {
    var copy = copyWatched();
    kill();
    
    for (var i = 0, len = copy.length; i < len; i += 1) {
      var o = copy[i];
      watch(o.file, o.action);
    }
    sys.puts('watchn reloaded\n');
  }
  
  function initialize() {
    var caller = process.argv[1];
    
    fs.watchFile(caller, {persistent: true, interval: 0}, function (curr, prev) {
      if (curr.mtime > prev.mtime) {
        reload();
      }
    });
...
// ----------------------------------------------------------------------------  
  ...
  watchn.help = help;
  watchn.notify = notify;     // kill this?
