...
Watchn.prototype.execute = function(task, options, reporter_name, show_pass, show_fail) {
  if (!task || !options) {
    throw new Error('watchn.execute requires a task and options to be passed in')
  }
  var self = this
  var key = reporter_name || 'basic'
  var sp = show_pass || false
  var sf = show_fail || true
  ...
  if (options.curr > options.prev) {
    exec(task, function(error, stdout, stderr) {
      ...
      self.notify(report.msg, {name: report.name || task, msg: report.gmsg}, show)
    })
  }
}
...
// TODO: Look at what's being returned from the map... should get the value
Watchn.prototype.createReporter = function(key) {
  ...
  var newsie = new reporter()
  this.reporters.add(key, newsie)
